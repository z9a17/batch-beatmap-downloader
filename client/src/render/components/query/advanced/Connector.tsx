  ...
  return (
    <div className="my-2 flex items-center">
      {types.map((type, index) => (
        <button
          className={classNames("w-12 border border-strong px-2 py-1 text-xs font-bold transition",
            { "border-r-0": index === 0 },
            { "border-l-0": index === types.length - 1 },
            { "bg-accent/20 text-accent": details.type === type },
            { "bg-panel-2 text-dim hover:text-ink": details.type !== type },
          )}
      ...
      <button
        className={classNames("ml-2 border border-strong px-2 py-1 text-xs font-bold transition",
          { "border-rose-400/30 bg-rose-400/15 text-rose-200": isNot },
          { "bg-panel-2 text-dim hover:text-ink": !isNot },
        )}
