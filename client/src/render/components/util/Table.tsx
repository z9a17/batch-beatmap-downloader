...
const Table: React.FC<PropTypes> = ({ data, headers, RenderRow = DefaultTableRow, className, onRowAction }) => (
  <table className={classNames(className, 'w-full text-mute')}>
      ...
      <tr>
        {headers.map((header, index) => (
          <th
            ...
            className={classNames(
              { 'pl-6': index === 0 },
              'px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-[0.08em] text-faint',
            )}
          >
            {header.title}
          </th>
    ...
    <tbody>
      {data.length === 0 ? <tr><td colSpan={6} className="px-6 py-8 text-center text-[13px] text-mute">No results found</td></tr> : (
        data.map((entry, index) => (
          <tr key={index} className={classNames(
            { 'border-b-0': index === data.length - 1 },
            'border-b border-line transition hover:bg-white/[0.025]'
          )}>
